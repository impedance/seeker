...
### Plan: ускорение UI (80/20)
- **Scope + non-goals:** минимизировать payload и DOM-операции для `loadSections` и `renderTree`, реализуя выборку только верхнего уровня секций, кэшированные `DocumentFragment` и делегированные события; не переписывать Filters/Export и не вносить глубокие изменения в API BaseX.
- **Affected entry points/files:** `app/js/api.js`, `app/js/navigation.js`, `app/js/ui.js`, `app/js/main.js`, `docs/status.md`, `docs/plan-ui-performance.md`, `docs/plan-template.md`.
- **Relevant AICODE anchors to read/update:** `docs/context.md` (`AICODE-CONTRACT: CONTRACT/DOCS_REQUIREMENT keep README/docs/context/docs/status/docs/decisions present [2025-10-05]`), `docs/status.md` (`AICODE-NOTE: STATUS/FOCUS`/`STATUS/ENTRY`), `docs/plan-ui-performance.md` (`AICODE-NOTE: NAV/UI_PERF_PLAN plan: ...`), `README.md` (если структура/entry points изменятся); после всех правок выполнить `rg -n "AICODE-" README.md docs/*.md`.
- **Risks/contracts to preserve:** не нарушать `AICODE-CONTRACT: CONTRACT/DOCS_REQUIREMENT ... [2025-10-05]`, не сломать `Navigation.history`/`breadcrumbs`, не добавлять новые `AICODE-*`, не ломать дерево/парсинг.
- **Test/check list:** измерить `loadSections` + `renderTree` (через `performance.now()`), убедиться, что делегированное дерево отвечает на клики/Enter/Space, проверить кэшированные DOM-фрагменты, убедиться в отсутствии ошибок DevTools, прогнать `rg -n "AICODE-" README.md docs/*.md`.
