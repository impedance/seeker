...
## [x] t13. Navigation: состояние и события (1-2ч)
...
## [x] t21. Search: debounce и интеграция поля (1-2ч)
**Файлы:** `app/js/search.js`, `app/js/main.js`
**Цель:** debounce ~300мс и обработчик `#search-input`.
**Готово:** ввод оборачивается в `search.js` с 300 мс задержки, `main.js` подписан на состояние и передает обновления в UI + навигацию.

## [x] t22. Search: XQuery и группировка (1-2ч)
**Файлы:** `app/js/search.js`
**Цель:** поиск по коду/названию/ресурсу и группировка по базам.
**Готово:** запрос ищет по работам и ресурсам, возвращает `Hit` с разделами и ресурсами, а результаты группируются по базам.

## [x] t23. Search: рендер результатов (1-2ч)
**Файлы:** `app/js/ui.js`, `app/js/search.js`
**Цель:** кликабельные результаты ведут в дерево.
**Готово:** в панель дерева выводятся группы, результаты показывают путь и ресурсы, клик поднимает дерево, раскрывает путь и выбирает работу.

---

# Фаза 6: Связи между базами

## [x] t24. API: цены ресурсов (1-2ч)
**Файлы:** `app/js/api.js`
**Цель:** запросы в `fsbts_mat`/`fsbts_mash` по коду.
**Готово:** добавлена функция поиска в каталог ресурсов, она кеширует цены и секции, и данные добавляются к списку ресурсов работы.

## [x] t25. UI: подсветка ресурсов с/без цены (1-2ч)
**Файлы:** `app/js/ui.js`, `app/css/style.css`
**Цель:** зеленый/красный статус.
**Готово:** строки таблицы получили цветовой фон и статусные метки, а колонка «Цена» показывает `cost` / `opt` или placeholder.

## [x] t26. Навигация к ресурсу из таблицы (1-2ч)
**Файлы:** `app/js/navigation.js`, `app/js/ui.js`
**Цель:** клик по ресурсу ведет в справочник.
**Готово:** клик по строке ресурса открывает карточку из fsbts (цена + категории), не сбрасывая дерево ГЭСН.

## [x] t27. История переходов (1-2ч)
**Файлы:** `app/js/navigation.js`
**Цель:** назад/вперед между Work и ресурсом.
**Готово:** реализована история с Back/Forward-кнопками, навигация возвращает на предыдущую работу или ресурс без дополнительных кликов.

---

# Фаза 7: Расширения

Этот этап объединяет расширения интерфейса, которые делают дерево более интерактивным (фильтры), позволяют вывести выборку наружу (экспорт) и закрепить часто используемые работы (избранное). Каждая задача описывает зависимые файлы и предполагаемый порядок действий, чтобы джун мог пройти шаги последовательно.
...
**План:**
1. Добавить в `app/index.html` панель фильтров (чекбоксы/тогглы) с data-атрибутами, которые описывают типы узлов (`work`, `resource`, `section` и т.п.).
2. В `app/js/ui.js` зацепить состояние фильтров и передать его в `renderTree`/`renderTreeNode`, чтобы ненужные узлы скрывались с помощью CSS (`hidden`/`aria-hidden`).
3. Сохранить последний выбор фильтров в `localStorage` или `sessionStorage` и загружать при инициализации.
...
**План:**
1. Собрать из текущего Work данные (имя, узлы, ресурсы, цены) в структурированный объект при клике по кнопке «Экспорт».
2. Добавить `app/js/export.js` и контроллер, который позволяет выбрать формат (JSON/CSV) и создать `Blob`, `URL.createObjectURL`, а затем автоматически кликнуть элемент `<a download>`.
3. Подключить `app/js/ui.js` к новым функциям, показывая уведомление об успешном экспорте и блокируя кнопку, пока данные загружаются.
...
**План:**
1. Создать API `Favorites` (в `app/js/favorites.js`) с методами `getList()`, `toggle(workId)` и `isFavorite(workId)`; использовать `localStorage` и JSON-строку.
2. Добавить в UI кнопку/иконку «В избранное» рядом с Work, подписать на события и обновить список слева (левый блок с базами/фильтрами).
3. Поддерживать навигацию к Work из списка избранного, чтобы клик раскрывал дерево и показывал детали.
